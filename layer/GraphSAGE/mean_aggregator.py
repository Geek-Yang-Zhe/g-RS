...
class MeanAggregator():
    def __init__(self, input_dim, output_dim, layer_idx, activation=tf.nn.relu, concat=False):
        ...
        with tf.variable_scope('gnn_layer_{}'.format(self.layer_id)):
            self.w_u = tf.get_variable(name='mean_agg_w_u', dtype=tf.float32,
                                       initializer=tf.random.uniform([self.input_dim, self.output_dim],
                                                                     -tf.sqrt(6.0  / (self.input_dim + self.output_dim)),
                                                                     tf.sqrt(6.0 / (self.input_dim + self.output_dim))))
    ...
    def call(self, inputs):
        ...
        if self.concat:
            outputs = tf.concat([mean_u_embedding, mean_v_embedding], axis=1)
        else:
            outputs = tf.add(mean_u_embedding, mean_v_embedding)
