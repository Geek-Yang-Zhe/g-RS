...
import layer.GraphSAGE.model.layers as layers
import layer.GraphSAGE.model.metrics as metrics
...
class SampleAndAggregate(GeneralizedModel):
    ...
    def __init__(self, placeholders, features, adj, degrees,
                 ...
                 model_size="small", identity_dim=0, lr=1e-5, neg_sample=19, weight_decay=0,
                 **kwargs):
        '''
        Args:
        ...
        if identity_dim > 0:
            self.embeds = tf.get_variable("node_embeddings", [adj.shape[0], identity_dim],
    ...
    def _build(self):
        ...
        neg_samples, neg_support_sizes = self.sample(self.neg_samples, self.layer_infos, self.neg_sample)
        self.neg_outputs, _ = self.aggregate(neg_samples, [self.features], self.dims, num_samples,
                                             neg_support_sizes, batch_size=self.neg_sample,
    ...
    def build(self):
        ...
        self.clipped_grads_and_vars = [(tf.clip_by_value(grad, -5.0, 5.0) if grad is not None else None, var)
                                  for grad, var in self.grads_and_vars]
        self.opt_op = self.optimizer.apply_gradients(self.clipped_grads_and_vars)
    ...
    def _loss(self):
        ...
        with tf.name_scope('loss'):
            self.affinity_u_v = tf.reduce_sum(self.outputs1 * self.outputs2, axis=1)  # [batch_size, ]
            self.affinity_u_neg = tf.matmul(self.outputs1, self.neg_outputs, transpose_b=True)  # [batch_size, num_neg]
    ...
    def _accuracy(self):
        ...
        with tf.name_scope('mrr'):
            ...
            total_samples = self.neg_sample + 1
            _, top_k_indices = tf.nn.top_k(concat_affinity, k=total_samples)
            _, self.rank1 = tf.nn.top_k(-top_k_indices, k=total_samples)
            self.rank1 = self.rank1[:, 0] + 1
        ...
        # self.neg_aff = tf.reshape(self.neg_aff, [self.batch_size, self.neg_sample])
